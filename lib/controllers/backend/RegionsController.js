RegionsController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('regions', this.getCriteria(), sort);
    },
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Regions.find(this.getCriteria(), sort);

        return this.render('regionsIndex', {
            data: {
    ...
    view: function() {
        return this.render('regionsView', {
            data: {
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Regions.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting Regions");
                Router.go('regionsView', {_id: _id});
            });
        }
        return this.render('regionsInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Regions.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating Regions");
            });
            Router.go('regionsView', {_id: _id});
        }
        return this.render('regionsUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Regions.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing Regions");
        });
    },
