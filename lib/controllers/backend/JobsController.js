JobsController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('jobs', this.getCriteria(), sort);
    },
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Jobs.find(this.getCriteria(), sort);

        return this.render('jobsIndex', {
            data: {
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Jobs.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting Jobs");
                Router.go('jobsView', {_id: _id});
            });
        }
        return this.render('jobsInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Jobs.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating Jobs");
            });
            Router.go('jobsView', {_id: _id});
        }
        return this.render('jobsUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Jobs.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing Jobs");
        });
    },
