CountriesController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('countries', this.getCriteria(), sort);
    },
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Countries.find(this.getCriteria(), sort);

        return this.render('countriesIndex', {
            data: {
    ...
    view: function() {
        return this.render('countriesView', {
            data: {
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Countries.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting Countries");
                Router.go('countriesView', {_id: _id});
            });
        }
        return this.render('countriesInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Countries.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating Countries");
            });
            Router.go('countriesView', {_id: _id});
        }
        return this.render('countriesUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Countries.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing Countries");
        });
    },
